...
describe("Cline Adapter", () => {
  ...
  it("should create mkdir action and write actions for each rule", () => {
    ...
    const actions = clineAdapter(input);
  ...
  it("should handle large number of rules", () => {
    ...
    expect(actions).toHaveLength(51); // 1 mkdir + 50 writes
  ...
  it("should use .clinerules directory", () => {
    const input: AdapterInput = {
    ...
    expect(actions[0].path).toBe(join(homedir(), "custom-path", ".clinerules"));
    expect(actions[1].path).toBe(
      join(homedir(), "custom-path", ".clinerules/test.md"),
    );
  });
});
